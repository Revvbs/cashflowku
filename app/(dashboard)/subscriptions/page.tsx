...
export default function SubscriptionsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [subRes, catRes, walletRes] = await Promise.all([
      supabase
        .from('subscriptions')
        .select('*, categories(id, name, icon, color), wallets(id, name, color)')
        .eq('user_id', user.id)
    ...
    const subs = (subRes.data ?? []).map((s: any) => ({
      ...s,
      categories: Array.isArray(s.categories) ? s.categories[0] ?? null : s.categories ?? null,
      wallets: Array.isArray(s.wallets) ? s.wallets[0] ?? null : s.wallets ?? null,
    }))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.next_billing_date) {
      setError('Tanggal tagihan berikutnya wajib diisi.')
      return
    }
    setSaving(true)
    setError('')
    ...
    if (editingSub) {
      const { error: updateErr } = await supabase
        ...
        .eq('user_id', user.id)
      if (updateErr) {
        setError('Gagal mengupdate langganan.')
        setSaving(false)
        return
      }
    } else {
      ...
      if (insertErr) {
        setError('Gagal menambahkan langganan.')
        setSaving(false)
        return
      }
    }
  ...
  async function handleToggleStatus(sub: SubscriptionRow) {
  ...
  function getSubLogo(sub: SubscriptionRow): string {
    const preset = PRESETS.find((p) => p.name.toLowerCase() === sub.name.toLowerCase())
    return preset?.icon ?? '📺'
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Lacak semua langganan bulananmu
          </p>
        </div>
        <button
        ...
        <>
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-xl p-5 md:p-6 mb-6 text-white">
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-xs text-indigo-200">Total / Bulan</p>
                <p className="text-lg font-bold mt-0.5">{formatCurrency(totalMonthly)}</p>
              </div>
              <div>
                <p className="text-xs text-indigo-200">Aktif</p>
                <p className="text-lg font-bold mt-0.5">{activeCount} langganan</p>
              </div>
              <div>
                <p className="text-xs text-indigo-200">Tagihan Berikutnya</p>
          ...
          <div className="space-y-3">
            {subscriptions.map((sub) => {
              ...
              return (
                <div
                  key={sub.id}
                  className="bg-white rounded-xl border border-gray-200 p-4 md:p-5 flex items-center gap-4 group hover:shadow-sm transition-shadow"
                >
                  {/* Logo */}
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={`inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${statusInfo.bg} ${statusInfo.text}`}>
                        {statusInfo.label}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-gray-500">
                      <span className="font-medium" style={{ color: catColor }}>
                        {formatCurrency(sub.amount)}{CYCLE_LABELS[sub.billing_cycle] ?? ''}
                      </span>
                      {sub.categories && (
                        <>
                    ...
                    <div className="flex items-center gap-1 mt-1 text-xs text-gray-400">
                      ...
                      <span>
                        {days >= 0
                          ? `Tagihan ${days === 0 ? 'hari ini' : `dalam ${days} hari`}`
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-8 md:p-12 text-center mb-6">
            ...
            <p className="text-sm text-gray-500 max-w-sm mx-auto mb-6">
              Tambahkan langgananmu agar tidak ada tagihan yang terlewat.
            </p>
            <button
              onClick={() => openAddModal()}
          ...
          {/* Quick Presets */}
          <div>
            <h2 className="text-sm font-semibold text-gray-900 mb-3">Langganan Populer</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {PRESETS.map((preset) => (
                <button
                  key={preset.name}
                  onClick={() => openAddModal(preset)}
                  className="bg-white rounded-xl border border-gray-200 p-4 text-left hover:shadow-sm hover:border-indigo-200 transition-all"
                >
                  <span className="text-2xl">{preset.icon}</span>
                  <p className="text-sm font-medium text-gray-900 mt-2">{preset.name}</p>
                  <p className="text-xs text-gray-500 mt-0.5">
                    {formatCurrency(preset.amount)}/bulan
                  </p>
                </button>
              ))}
            </div>
          ...
          <div className="relative bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
              <h2 className="text-base font-semibold text-gray-900">
                {editingSub ? 'Edit Langganan' : 'Tambah Langganan'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Siklus</label>
                  <select
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Tagihan Berikutnya</label>
                <input
                  type="date"
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {saving ? 'Menyimpan...' : editingSub ? 'Simpan' : 'Tambah'}
                </button>
              </div>
            </form>
          </div>
